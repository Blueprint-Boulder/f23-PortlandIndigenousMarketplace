...
export default class MockLoginService {
  // Attempts to login the user. Returns a Vendor object on success,
  // or null on failure
  ...
  // Attempts to register the user. Returns true on success,
  // and false on failure.
  static register({name, email, password, website, phoneNumber}) {
    // Check if email is already in use
    if (MockVendorService.getVendorByEmail(email) != null) return false;

    // Create the vendor
    const v = new Vendor(
        MockVendorService.getLastVendorId() + 1,
        name, email, website, phoneNumber);
